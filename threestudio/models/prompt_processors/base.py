...
from threestudio.utils.animportrait3d_prompt import (
    view_direction_indices as animportrait3d_view_direction_indices,
    view_prompt as animportrait3d_view_prompt,
)
...
class PromptProcessorOutput:
    ...
    null_embeddings: Float[Tensor, "N Nf"]
    ...
    def get_view_direction_indices(
        self,
    ...
    def get_text_embeddings(
        ...
        if view_dependent_prompting:
            # Get direction
            direction_idx = self.get_view_direction_indices(
...
class PromptProcessor(BaseObject):
    ...
    class Config(BaseObject.Config):
        ...
        front_threshold: Tuple[float, float] = (45, 135)
        back_threshold: Tuple[float, float] = (-135, -45)
        view_dependent_prompt_front: bool = False
        # ``animportrait3d`` uses exactly "{direction} view {prompt}" and the
        # strict yaw bins from GSAvatar instead of threestudio's generic style.
        view_prompt_style: str = "default"
        use_cache: bool = True
        spawn: bool = True
    ...
    def configure(self) -> None:
        ...
        view_prompt_style = str(self.cfg.view_prompt_style).strip().lower()
        if view_prompt_style == "animportrait3d":
            if bool(self.cfg.use_perp_neg):
                raise ValueError(
                    "AnimPortrait3D view prompts do not support the four-view "
                    "Perp-Neg interpolation"
                )
            self.directions = [
                DirectionConfig(
                    "side",
                    lambda s: animportrait3d_view_prompt("side", s),
                    lambda s: s,
                    lambda ele, azi, dis: (
                        animportrait3d_view_direction_indices(azi) == 0
                    ),
                ),
                DirectionConfig(
                    "front",
                    lambda s: animportrait3d_view_prompt("front", s),
                    lambda s: s,
                    lambda ele, azi, dis: (
                        animportrait3d_view_direction_indices(azi) == 1
                    ),
                ),
                DirectionConfig(
                    "back",
                    lambda s: animportrait3d_view_prompt("back", s),
                    lambda s: s,
                    lambda ele, azi, dis: (
                        animportrait3d_view_direction_indices(azi) == 2
                    ),
                ),
            ]
        elif view_prompt_style != "default":
            raise ValueError(
                "view_prompt_style must be 'default' or 'animportrait3d', "
                f"got {self.cfg.view_prompt_style!r}"
            )
            ...
            self.directions = [
                ...
                DirectionConfig(
                    ...
                    lambda s: s,
                    lambda ele, azi, dis:
                    (shift_azimuth_deg(azi) > self.cfg.front_threshold[0])
                    & (shift_azimuth_deg(azi) < self.cfg.front_threshold[1]),
                ),
                DirectionConfig(
                    "back",
                    ...
                    lambda s: s,
                    lambda ele, azi, dis:
                    (shift_azimuth_deg(azi) > self.cfg.back_threshold[0])
                    & (shift_azimuth_deg(azi) < self.cfg.back_threshold[1]),
                ),
                DirectionConfig(
                    "overhead",
    ...
    def load_text_embeddings(self):
        ...
        self.null_embeddings = self.load_from_cache("")[None, ...]
