...
class Avatar:
    def __init__(self, ply_path):
        ...
        gaussian = GaussianFlameUVModel(sh_degree=0)
        ...
        cameras_extent = 4.0
        flame_scale = -10.0
        gaussian.initialize_flame_state(cameras_extent, flame_scale)
    ...
    def get_camera(self, dist, elev, azim, fovy_deg=50.0):
...
class NerSemble:
    def __init__(self, exp_path, pose_path):
        ...
        azimuth_deg = torch.tensor([90, ], dtype=torch.float, device=device).repeat(self.n_frames)
...
def batch_gs_render(flame_sequences, save_path, static=False, dynamic=False, eye_pose=False):
    ...
    flame_path = osp.join(save_path, 'flame_cond')
    ...
    for i in tqdm(range(n_views)):
        ...
        if not eye_pose:
            leye_pose = None
            reye_pose = None
        avatar.set_pose(expression=expression, jaw_pose=jaw_pose, neck_pose=neck_pose, leye_pose=leye_pose, reye_pose=reye_pose)
...
def batch_mesh_render(flame_sequences, static=False, dynamic=False):
    ...
    for i in tqdm(range(len(flame_sequences))):
        ...
        image = image.clip(min=0, max=1)
        image = (image * 255.0).astype(np.uint8)
        imageio.imwrite(os.path.join(flame_path, f"flame_{i:06d}.png"), image)
        images.append(image)
    images = np.stack(images, axis=0)
    return images
...
if __name__ == '__main__':
    ...
    ply_path = 'outputs/vsd_test/20260611-024430/optim2/ckpt/epoch_0009.ply'
