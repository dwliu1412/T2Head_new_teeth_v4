...
def main(args, extras) -> None:
    ...
    # Optional selective registry population.  This avoids importing unrelated
    # geometry/material stacks (and tiny-cuda-nn) for the reconstructed-head
    # Stage-2 path.  Legacy eager behavior remains the default.
    if os.environ.get("THREESTUDIO_LAZY_IMPORT", "0") == "1":
        registry_modules = {
            "reconstruction-finetune-datamodule": (
                "threestudio.data.reconstruction_finetune"
            ),
            "random-camera-exp-datamodule": "threestudio.data.uncond_rand_exp",
            "head-3dgs-reconstruction-finetune-system": (
                "threestudio.systems.Head3DGSLKsFinetune"
            ),
            "head-3dgs-lks-rig-system": "threestudio.systems.Head3DGSLKs",
        ...
        requested_components = [
            str(cfg.data_type),
            str(cfg.system_type),
            str(cfg.system.get("guidance_type", "")),
            str(cfg.system.get("prompt_processor_type", "")),
        ]
        for component in requested_components:
            if not component:
                continue
            module_name = registry_modules.get(component)
            if module_name is not None:
                importlib.import_module(module_name)
            if component not in threestudio.__modules__:
                raise KeyError(
                    "THREESTUDIO_LAZY_IMPORT=1 has no selective import mapping "
                    f"for registry component {component!r}. Either add the "
                    "mapping in launch.py or unset the environment variable."
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", required=True, default="configs/t2head.yaml", help="path to config file")
    parser.add_argument(
        "--gpu",
        default="0",
