...
class FlameHead(nn.Module):
    ...
    def add_teeth(self):
        ...
        shape_dirs_mean = (self.shapedirs[vid_lip_outside_ring_upper, :, :self.n_shape_params] + self.shapedirs[
            vid_lip_outside_ring_lower, :, :self.n_shape_params]) / 2
    ...
    def add_oral_cavity(self):
        """Append a small rigid back wall behind the explicit teeth.

        The old Stage-2 representation synthesized a cavity by moving dark
        Gaussians away from lip/teeth faces along their normals.  That shell
        can cross the lips when the jaw moves.  A real mesh patch gives cavity
        Gaussians their own surface and UV chart, so their normal residual can
        stay exactly zero.  The wall follows the head/neck joint as one rigid
        layer.  Mixing neck and jaw weights on connected faces would stretch
        their tangent Jacobian into long streaks whenever the mouth opens;
        lower teeth already provide the jaw-attached moving layer.
        """

        upper_lip = self.mask.get_vid_by_region(
            "lip_outside_ring_upper", keep_order=True
        )
        lower_lip = self.mask.get_vid_by_region(
            "lip_outside_ring_lower", keep_order=True
        )
        if upper_lip.numel() != lower_lip.numel() or upper_lip.numel() < 2:
            raise RuntimeError("FLAME lip rings cannot define an oral cavity wall")

        columns = int(upper_lip.numel())
        upper = self.v_template[upper_lip]
        lower = self.v_template[lower_lip]
        center = 0.5 * (upper + lower)
        mouth_height = (upper - lower).norm(dim=-1).mean()
        center[:, 1] = center[:, 1].mean()
        center[:, 2] -= 3.2 * mouth_height

        vertical_extent = 2.2 * mouth_height
        top = center.clone()
        middle = center.clone()
        bottom = center.clone()
        top[:, 1] += vertical_extent
        bottom[:, 1] -= vertical_extent
        cavity_vertices = torch.cat((top, middle, bottom), dim=0)

        vertex_offset = int(self.v_template.shape[0])
        cavity_vertex_ids = torch.arange(
            vertex_offset,
            vertex_offset + cavity_vertices.shape[0],
            dtype=torch.long,
            device=self.v_template.device,
        )
        self.v_template = torch.cat((self.v_template, cavity_vertices), dim=0)
        self.mask.v.register_buffer("oral_cavity", cavity_vertex_ids)

        u = torch.linspace(0.64, 0.36, columns, device=self.verts_uvs.device)
        v = torch.tensor(
            [0.945, 0.925, 0.905],
            dtype=self.verts_uvs.dtype,
            device=self.verts_uvs.device,
        )
        cavity_uv = torch.stack(
            torch.meshgrid(v, u, indexing="ij"), dim=-1
        )[..., [1, 0]].reshape(-1, 2)
        uv_offset = int(self.verts_uvs.shape[0])
        self.verts_uvs = torch.cat((self.verts_uvs, cavity_uv), dim=0)
        ...
        for row in range(2):
            ...
            for column in range(columns - 1):
                ...
                c = next_start + column
                d = c + 1
                face_rows.extend(((a, c, b), (b, c, d)))
        cavity_faces = torch.tensor(
            face_rows, dtype=torch.long, device=self.faces.device
        )

        shape_dirs_mean = 0.5 * (
            self.shapedirs[upper_lip, :, : self.n_shape_params]
            + self.shapedirs[lower_lip, :, : self.n_shape_params]
        )
        cavity_shapedirs = torch.zeros(
            (
                cavity_vertices.shape[0],
                self.shapedirs.shape[1],
                self.shapedirs.shape[2],
            ),
            dtype=self.shapedirs.dtype,
            device=self.shapedirs.device,
        )
        cavity_shapedirs[:, :, : self.n_shape_params] = shape_dirs_mean.repeat(
            3, 1, 1
        )
        self.shapedirs = torch.cat((self.shapedirs, cavity_shapedirs), dim=0)

        pose_basis = len(self.parents) - 1
        old_vertex_count = vertex_offset
        posedirs = self.posedirs.reshape(pose_basis, 9, old_vertex_count, 3)
        posedirs = torch.cat(
            (
                posedirs,
                torch.zeros(
                    (pose_basis, 9, cavity_vertices.shape[0], 3),
        ...
        self.J_regressor = torch.cat(
            (
                self.J_regressor,
                torch.zeros(
                    (self.J_regressor.shape[0], cavity_vertices.shape[0]),
        ...
        cavity_weights = torch.zeros(
            (cavity_vertices.shape[0], self.lbs_weights.shape[1]),
            dtype=self.lbs_weights.dtype,
            device=self.lbs_weights.device,
        )
        cavity_weights[:, 1] = 1.0
        self.lbs_weights = torch.cat((self.lbs_weights, cavity_weights), dim=0)

        self.faces = torch.cat((self.faces, cavity_faces + vertex_offset), dim=0)
        self.textures_idx = torch.cat(
            (self.textures_idx, cavity_faces + uv_offset), dim=0
        )
        self.mask.update(self.faces, self.textures_idx)
